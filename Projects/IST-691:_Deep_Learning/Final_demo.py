...
def calc_landmark_list(W,H, landmarks): # Get landmarks list
    '''
    Change landmarks cordinates to lists.
    '''
    image_width = W
    image_height = H
...
def pre_process_landmark(landmark_list): # Pre-process landmark lists
    '''
    Preprocess raw landmark. Convert to relative coordinates and normalize coordinates.
    '''
...
def update_history (last_char,prediction,character_count,limit): # Calculate predictions count
    '''
    Check if the software needs to write the letter into the textbox.
    '''
    flag = 0
    # count how many times the same letter is predicted
    ...
    # define if the prediction needs to be activate (written or do an action like delete)
    ...
    return last_char,character_count, flag

def speak(): # Say it

    '''
    Say the text box message.
    '''
...
def update_frame(last_char, character_count): # Update Frame
    ...
    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) # transform to RGB
    results = hands.process(frame_rgb) # get landamarks from mediapipe
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            landmark_list = calc_landmark_list(W,H, hand_landmarks) # get landamrk list
            #print(landmark_list)
        ...
        flag = 0
        if confidence > 70:
            last_char, character_count, flag = update_history(last_char, pred_character, character_count, limit)

        if flag == 1: # draw a white box if a letter is written
            cv2.rectangle(frame_rgb, (min_x - 10, min_y - 10), (max_x + 10, max_y + 10), (255, 255, 255), -1)
...
# Create textbox space
text_display = tk.Text(root, width=40, height=2, font=("Arial", 18))
text_display.grid(row=1, column=0, columnspan=3, padx=5, pady=5)

# Buttons for clear, say, and quit
clear_button = tk.Button(root, text="Clear All", width=20, bg="lemon chiffon", command=lambda: text_display.delete("1.0", tk.END), font=("Arial", 18))
clear_button.grid(row=2, column=0, padx=5, pady=5)

speak_button = tk.Button(root, text="Say it", width=20, bg="green",fg="white", command=lambda: speak(), font=("Arial", 18))
speak_button.grid(row=2, column=1, padx=5, pady=5)

quit_button = tk.Button(root, text="Quit", width=15, bg="red",fg="white", command=root.quit, font=("Arial", 18))
